...
for (let dayIndex = 0; dayIndex < 5; dayIndex += 1) {
  ...
  assert.deepEqual(tasks.map((task) => task.slot), [0, 1, 2, 3, 4, 5, 6, 7]);
...
assert.deepEqual(JSON.parse(JSON.stringify(api.normalizeOfficeState(null))), { version: 1, completed: {}, attempts: {} });
...
assert.deepEqual(api.availableTasks(state).map((task) => task.id), [firstMonday.id]);
state.minute = secondMonday.unlockMinute;
assert.deepEqual(api.availableTasks(state).map((task) => task.id), [firstMonday.id, secondMonday.id]);
state.metadata.officeWork.completed[firstMonday.id] = { minute: state.minute };
assert.deepEqual(api.availableTasks(state).map((task) => task.id), [secondMonday.id]);
assert.deepEqual(api.completedForDay(state).map((task) => task.id), [firstMonday.id]);
...
assert.ok(html.indexOf("src/office-work-pack.js") < html.indexOf("src/bootstrap.js"));
