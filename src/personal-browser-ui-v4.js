(function (root) {
  ...
  if (root.UntilFridayPersonalBrowserUIV4) return;
  ...
  const ROUTES = {
    ...
    min: { title: "МИН", url: "https://min.local/" },
  ...
  const DEFAULT_BOOKMARKS = ["market", "videotok", "min"];
  ...
  function pageFromAddress(value) {
    ...
    if (text.includes("min.local") || text === "мин" || text.includes("мессенджер мин")) return "min";
  ...
  function appIcon(page) {
    const names = { home: "home", market: "shopping-bag", videotok: "play-button-circled", min: "chat", history: "time-machine", downloads: "download", settings: "settings", search: "search" };
    return uiIcon(names[page] || "internet", 16);
  }

  function browserChrome(user, current) {
    const tab = activeTab();
    const bookmarked = user.bookmarks.includes(current.page);
    return `<div class="real-browser ${user.settings.compactMode ? "compact" : ""}" data-browser-v4-shell>
  ...
  function homePage(state, user) {
    ...
    const unread = root.UntilFridayMinMessenger?.unreadCount?.() || 0;
    const quick = [
      ["market", "КупиТут", "Покупки"],
      ["videotok", "Видеоток", "Видео и подписки"],
      ["min", "МИН", unread ? `${unread} непрочитанных` : "Минимум лишнего"],
      ["history", "История", "Недавние страницы"]
    ];
    return `<section class="rb-newtab"><div class="rb-search-brand"><span>K</span><strong>${esc(user.settings.searchEngine)}</strong></div><form class="rb-main-search" data-rb-search><span>${uiIcon("search", 22)}</span><input placeholder="Введите запрос или адрес"><button>Найти</button></form><div class="rb-quick">${quick.map(([page, label, note]) => `<button data-rb-page="${page}"><span>${appIcon(page)}</span><b>${label}</b><small>${note}</small></button>`).join("")}</div><div class="rb-home-grid"><section><header><h2>Недавние страницы</h2><button data-rb-page="history">Вся история</button></header>${recent.length ? recent.map((item) => `<button class="rb-recent" data-rb-address-value="${esc(item.url || "")}"><span>${appIcon(item.category === "market" ? "market" : item.category === "videotok" ? "videotok" : item.category === "min" ? "min" : "home")}</span><div><b>${esc(item.title)}</b><small>${esc(item.site || item.url || "")}</small></div><time>${DAY_SHORT[item.dayIndex] || ""} ${formatTime(item.minute)}</time></button>`).join("") : `<p class="rb-empty">Здесь появятся недавно посещённые страницы.</p>`}</section><section><header><h2>Быстрый доступ</h2></header><div class="rb-suggestions"><button data-rb-page="market"><span>${appIcon("market")}</span><div><small>КупиТут</small><b>Продолжить просмотр каталога</b><em>Маркетплейс</em></div></button><button data-rb-page="min"><span>${appIcon("min")}</span><div><small>МИН</small><b>${unread ? `${unread} непрочитанных сообщений` : "Открыть мессенджер"}</b><em>Чаты · группы · каналы</em></div></button></div></section></div></section>`;
  }
  ...
  function settingsPage(user) {
    ...
    return internalPage("settings", `<header><div><h1>Настройки</h1><p>Параметры профиля браузера.</p></div></header><section class="rb-settings"><h2>При запуске</h2><label><div><b>Домашняя страница</b><small>Открывается по кнопке «Домой».</small></div><select data-rb-setting="homePage"><option value="home" ${settings.homePage === "home" ? "selected" : ""}>Новая вкладка</option><option value="market" ${settings.homePage === "market" ? "selected" : ""}>КупиТут</option><option value="videotok" ${settings.homePage === "videotok" ? "selected" : ""}>Видеоток</option><option value="min" ${settings.homePage === "min" ? "selected" : ""}>МИН</option></select></label></section><section class="rb-settings"><h2>Внешний вид</h2>${toggle("showBookmarksBar", "Показывать панель закладок", "Сохранённые сайты под адресной строкой.")}${toggle("compactMode", "Компактный интерфейс", "Уменьшить высоту панелей.")}</section><section class="rb-settings"><h2>Безопасность</h2>${toggle("safeSearch", "Безопасный поиск", "Скрывать нежелательные материалы.")}</section>`);
  }
  ...
  function searchPage(user, query) {
    ...
    const min = root.UntilFridayMinMessenger?.search?.(query) || { chats: [], messages: [], users: [] };
    const count = products.length + videos.length + min.chats.length + min.messages.length + min.users.length;
    return `<section class="rb-search-results"><header><div><span>K</span><b>${esc(user.settings.searchEngine)}</b></div><form data-rb-search><input value="${esc(query)}"><button>Найти</button></form></header><main><p>Найдено результатов: ${count}</p>${products.map((item) => result("kupitut.local", item.title, item.note, "market")).join("")}${videos.map((item) => result("videotok.local", item.title, `${item.channelName} · ${item.duration}`, "videotok", `${root.UntilFridayVideotok.ROUTES.watch}${item.id}`)).join("")}${min.chats.map((chat) => result("min.local", chat.title, chat.description || "Чат в МИН", "min", `https://min.local/chat/${chat.id}`)).join("")}${min.messages.map((message) => result("min.local", root.UntilFridayMinMessenger.chatById(message.chatId)?.title || "Сообщение", message.text, "min", `https://min.local/chat/${message.chatId}`)).join("")}${min.users.map((userItem) => result("min.local", userItem.name, `@${userItem.username}`, "min")).join("")}${count ? "" : `<div class="rb-empty large"><h2>Ничего не найдено</h2></div>`}</main></section>`;
  }
  ...
  function renderPage(page, state, user, current) {
    ...
    if (current.page === "min") {
      const api = root.UntilFridayMinMessenger;
      if (!api) { page.innerHTML = `<div class="rb-empty"><h2>МИН не загрузился</h2></div>`; return; }
      api.mount(page, { url: current.url, navigate(url, title) { navigate("min", { url, title }); } });
      return;
    }
    if (current.page === "history") page.innerHTML = historyPage(user);
    ...
    else if (current.page === "search") page.innerHTML = searchPage(user, current.query);
  ...
  function render() {
    ...
    win.querySelector(".window-content").innerHTML = browserChrome(user, current);
  ...
  function toggleBookmark() {
    const current = entry();
    if (!["market", "videotok", "min"].includes(current.page)) return;
  ...
  function settingAction(control) {
    ...
    updatePersonal((valueState) => { valueState.settings[key] = value; }, "personal-browser-setting");
  }
  ...
  root.addEventListener?.("until-friday-state-change", (event) => { if (event.detail?.reason === "time") return; schedule(); });
  ...
  root.UntilFridayPersonalBrowserUIV4 = {
    ROUTES,
    DEFAULT_SETTINGS,
    navigate,
    navigateAddress,
